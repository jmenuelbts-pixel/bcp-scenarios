...
const CHAMPS = 'id, email, prenom, nom, date_naissance, role, entreprise, statut, classe_id, created_at'
...
export async function definirStatut(
  ...
  const maj: { statut: string; classe_id?: string | null } = { statut }
  if (statut === 'accepte' && classeId) maj.classe_id = classeId
  const { error } = await supabase.from('profiles').update(maj).eq('id', eleveId)
