...
// chiffres + lettres, bénéficiaire, titulaire, lieu/date, ligne MICR.
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18, margin: '10px 0' }}>
      {cheques.map((c, i) => {
        ...
        return (
          <div key={i} style={{ position: 'relative', background: '#FBFCFB', border: `1px solid ${c.couleur}33`, borderRadius: 6, overflow: 'visible', boxShadow: '0 1px 5px rgba(0,0,0,0.13)', marginTop: (c.postitAnnee || c.postitAccessoire) ? 14 : 0 }}>
            {(c.postitAnnee || c.postitAccessoire) && (
              <div style={{ position: 'absolute', top: -14, right: 16, zIndex: 2, background: '#FFF7B0', padding: '7px 11px', boxShadow: '0 3px 7px rgba(0,0,0,0.22)', transform: 'rotate(3deg)', fontFamily: '"Bradley Hand","Segoe Print","Comic Sans MS",cursive', fontSize: 12.5, color: '#3A3A1E', lineHeight: 1.35, maxWidth: 180, borderRadius: 2 }}>
                {c.postitAnnee && <div style={{ fontWeight: 700 }}>Véhicule acheté en {c.postitAnnee}</div>}
                {c.postitAccessoire && <div>{c.postitAccessoire}</div>}
              </div>
            )}
            <div style={{ position: 'absolute', inset: 0, background: `repeating-linear-gradient(115deg, ${c.couleur}0C 0 10px, transparent 10px 20px)`, pointerEvents: 'none', borderRadius: 6 }} />
            ...
            <div style={{ position: 'relative', padding: '14px 18px', display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10 }}>
                ...
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  ...
                  {c.montant && <div style={{ border: `1.5px solid ${c.couleur}`, borderRadius: 3, padding: '4px 12px', fontWeight: 700, fontSize: 14, marginTop: 2, color: '#1F2933' }}>{c.montant}</div>}
                </div>
              </div>
