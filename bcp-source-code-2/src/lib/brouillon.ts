...
export interface Enregistreur<T> {
  sauver(donnees: T): void
  flush(): void
  annuler(): void
}
...
// Hook : force l'ecriture distante du brouillon des que l'onglet est masque,
// que la page se ferme, ou que l'appli passe en arriere-plan (mobile inclus).
// Le localStorage est deja ecrit a chaque frappe ; ici on pousse aussi vers
// Supabase pour ne rien perdre meme si l'eleve ne revient jamais sur ce poste.
// A appeler dans chaque onglet de saisie avec la ref de l'enregistreur.
export function useFlushBrouillon(ref: { current: Enregistreur<unknown> | Enregistreur<never> | { flush(): void } }): void {
  useEffect(() => {
    const flush = () => {
      try {
        ref.current.flush()
