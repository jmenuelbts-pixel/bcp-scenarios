...
export function Inscriptions() {
  ...
  const [choixClasse, setChoixClasse] = useState<Record<string, string>>({})

  async function charger() {
    setChargement(true)
    const [liste, cl] = await Promise.all([listerDemandes(), listerClasses()])
    setDemandes(liste)
    setClasses(cl)
    setChargement(false)
  }
  ...
  async function traiter(eleveId: string, statut: 'accepte' | 'refuse') {
    ...
    const classeId = statut === 'accepte' ? choixClasse[eleveId] ?? null : null
    const { erreur } = await definirStatut(eleveId, statut, classeId)
    if (!erreur) {
            ...
            {demandes.map((e) => (
              ...
              >
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                  <select
                    value={choixClasse[e.id] ?? ''}
                    onChange={(ev) => setChoixClasse((c) => ({ ...c, [e.id]: ev.target.value }))}
                    title="Classe de l'élève"
                    style={{ fontFamily: 'Arial, sans-serif', border: '1px solid #C9D6E3', borderRadius: 8, padding: '8px 10px', fontSize: 13, minWidth: 160 }}
                  >
