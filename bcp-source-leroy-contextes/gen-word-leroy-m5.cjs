...
function consignes() {
  return [
    ...
    P("Nous sommes 7 jours après la signature du bon de commande et votre tutrice, Mme Annie Mâle, vous demande de sélectionner sur la liste des artisans travaillant avec Leroy Merlin, celui qui réalisera la prestation.", { it: true }),
    ...
    P("Le dressing a été installé depuis quelques jours et votre responsable vous demande de contacter Mme et M. Sankouraga par téléphone pour savoir comment s'est passée la prestation.", { it: true }),
    P("6. Préparez votre appel téléphonique en utilisant la méthode CROC. (Consulter le document 4, compléter l'annexe 6)."),
    P("Vous n'avez pas réussi à joindre directement le couple par téléphone. Vous avez laissé un message à Mme Sankouraga sur son smartphone et elle vous a répondu sur votre mail dont l'adresse est (conseillerventestagiaire@leroymerlin.fr).", { it: true }),
