...
export function OngletJournal({ couleur, etudiantId, missionId }: Props) {
  ...
  useEffect(() => {
    ...
    chargerJournal(etudiantId, missionId).then((j) => {
      ...
      // Un brouillon plus recent qu'un eventuel enregistrement prend le dessus.
      lireBrouillon<{ nonReussi: string; moinsBien: string }>(
        etudiantId,
        missionId,
        'journal'
      ).then((b) => {
        if (!actif || !b) return
        if (typeof b.nonReussi === 'string') setNonReussi(b.nonReussi)
        if (typeof b.moinsBien === 'string') setMoinsBien(b.moinsBien)
      })
    })
    return () => {
      actif = false
    ...
    <div style={{ fontFamily: 'Arial, sans-serif' }}>
      ...
      <textarea
        ...
        onChange={(e) => {
          ...
          setEnregistre(false)
          sauverBrouillon(etudiantId, missionId, 'journal', { nonReussi: e.target.value, moinsBien })
        }}
        style={champ}
      />
      ...
      <textarea
        ...
        onChange={(e) => {
          ...
          setEnregistre(false)
          sauverBrouillon(etudiantId, missionId, 'journal', { nonReussi, moinsBien: e.target.value })
        }}
        style={champ}
      />
