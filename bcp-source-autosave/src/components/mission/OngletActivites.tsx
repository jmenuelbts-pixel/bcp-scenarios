...
function VueFlashcards({
  ...
  useEffect(() => {
    ...
    chargerQuiz(etudiantId, missionId, 'flashcards').then((s) => {
      ...
      lireBrouillon<{ vues: number[] }>(etudiantId, missionId, 'flashcards').then((b) => {
        if (!actif || !b || !Array.isArray(b.vues)) return
        setVues(new Set(b.vues))
      })
    })
    return () => {
      actif = false
  ...
  async function envoyer() {
    ...
    if (erreur) setErreur('L envoi a echoue. Veuillez reessayer.')
      ...
      setVerrouille(true)
      void effacerBrouillon(etudiantId, missionId, 'flashcards')
    }
    setEnCours(false)
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {contenu.flashcards.map((f, i) => (
        <CarteFlash key={i} carte={f} couleur={couleur} onVue={() => setVues((s) => { const maj = new Set(s).add(i); sauverBrouillon(etudiantId, missionId, 'flashcards', { vues: [...maj] }); return maj })} />
      ))}
...
function VueQuiz({
  ...
  useEffect(() => {
    ...
    chargerQuiz(etudiantId, missionId, 'quiz').then((soumission) => {
      ...
      lireBrouillon<ReponsesEleve>(etudiantId, missionId, 'quiz').then((b) => {
  ...
  function definir(i: number, valeur: number[] | string[]) {
    ...
    setReponses((r) => {
      ...
      sauverBrouillon(etudiantId, missionId, 'quiz', maj)
      return maj
    })
    setEnvoye(false)
  }
  ...
  async function envoyer() {
    ...
    if (etudiantId) {
      ...
      if (erreur) setErreur('L enregistrement du resultat a echoue.')
        ...
        setVerrouille(true)
        void effacerBrouillon(etudiantId, missionId, 'quiz')
      }
      setEnCours(false)
    }
  }
...
function VueGlisser({
  ...
  useEffect(() => {
    ...
    chargerQuiz(etudiantId, missionId, 'glisser').then((s) => {
      ...
      lireBrouillon<Record<number, number>>(etudiantId, missionId, 'glisser').then((b) => {
        if (!actif || !b || typeof b !== 'object') return
        setChoix(b)
      })
    })
  ...
  async function envoyer() {
    ...
    if (erreur) setErreur('L envoi a echoue. Veuillez reessayer.')
      ...
      setVerrouille(true)
      void effacerBrouillon(etudiantId, missionId, 'glisser')
    }
    setEnCours(false)
  }
          ...
          <div key={i} style={{ border: '1px solid #DCE8F4', borderRadius: 10, padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', background: '#FFFFFF' }}>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setChoix((c) => { const maj = { ...c, [i]: Number(v) }; sauverBrouillon(etudiantId, missionId, 'glisser', maj); return maj })
              }}
              style={{
                fontFamily: 'Arial, sans-serif',
                fontSize: 13,
