...
function rendreAnnexe(
  ...
  if (annexe.type === 'cap') return rendreCap(annexe, saisies, set, verrouille, couleur)
...
// Fiche produit : interface type logiciel pro (sections technique / equipements / commercial)
function rendreFicheProduit(a: AnnexeFicheProduit, saisies: Saisies, set: (id: string, v: string) => void, verrouille: boolean, couleur: string) {
  const inp: React.CSSProperties = { width: '100%', boxSizing: 'border-box', border: '1px solid #D5DBE1', borderRadius: 6, padding: '6px 9px', fontFamily: 'Arial, sans-serif', fontSize: 13.5, color: verrouille ? '#6B7280' : '#1F2933', outline: 'none', background: verrouille ? '#F3F4F6' : '#FFFFFF' }
  const section = (titre: string) => (
    <div style={{ background: couleur, color: '#FFFFFF', padding: '7px 12px', fontSize: 12.5, fontWeight: 700, letterSpacing: 0.3, textTransform: 'uppercase' }}>{titre}</div>
  )
  const champLib = (lib: string, champId: string) => (
    <div key={champId} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 12px', borderBottom: '1px solid #F1F3F5' }}>
      <label style={{ flex: '0 0 42%', fontSize: 13, color: '#4B5563', fontWeight: 600 }}>{lib}</label>
      <input type="text" disabled={verrouille} value={saisies[champId] ?? ''} onChange={(e) => set(champId, e.target.value)} style={inp} />
    </div>
  )
  const equipements = []
  for (let i = 0; i < a.nbEquipements; i++) equipements.push(i)
  return (
    <div style={{ border: '1px solid #DCE8F4', borderRadius: 10, overflow: 'hidden', boxShadow: '0 1px 3px rgba(20,40,70,0.06)' }}>
      <div style={{ background: '#16456E', color: '#FFFFFF', padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 10 }}>
        <span style={{ fontSize: 16 }}>🚗</span>
        <span style={{ fontSize: 14, fontWeight: 700 }}>{a.titre}</span>
      </div>
      {section('Fiche technique')}
      <div style={{ background: '#FFFFFF' }}>
        {a.technique.map((l) => champLib(l.libelle, `${a.id}.t.${l.id}`))}
      </div>
      {section('Équipements')}
      <div style={{ background: '#FFFFFF', padding: '8px 12px', display: 'flex', flexDirection: 'column', gap: 6 }}>
        {equipements.map((i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ color: couleur, fontWeight: 700 }}>•</span>
            <input type="text" disabled={verrouille} value={saisies[`${a.id}.e.${i}`] ?? ''} onChange={(e) => set(`${a.id}.e.${i}`, e.target.value)} placeholder={`Équipement ${i + 1}`} style={inp} />
          </div>
        ))}
      </div>
      {section('Caractéristiques commerciales')}
      <div style={{ background: '#FFFFFF' }}>
        {a.commercial.map((l) => champLib(l.libelle, `${a.id}.c.${l.id}`))}
      </div>
    </div>
  )
}

// Tableau C.A.P. : Mobile / Caracteristique / Avantage / Preuve a saisir
function rendreCap(a: AnnexeCap, saisies: Saisies, set: (id: string, v: string) => void, verrouille: boolean, couleur: string) {
  const lignes = []
  for (let i = 0; i < a.nbLignes; i++) lignes.push(i)
  const cols = [
    { k: 'mobile', t: "Mobile d'achat" },
    { k: 'carac', t: 'Caractéristique' },
    { k: 'avantage', t: 'Avantage' },
    { k: 'preuve', t: 'Preuve' },
  ]
  const inp: React.CSSProperties = { width: '100%', boxSizing: 'border-box', border: '1px solid #D5DBE1', borderRadius: 6, padding: '6px 8px', fontFamily: 'Arial, sans-serif', fontSize: 13, color: verrouille ? '#6B7280' : '#1F2933', outline: 'none' }
  return (
    <div style={{ border: '1px solid #DCE8F4', borderRadius: 8, overflow: 'hidden' }}>
      <div style={{ background: '#EEF3F8', padding: '6px 10px', fontSize: 13, fontWeight: 700, color: '#16456E' }}>{a.titre}</div>
      <div style={{ overflowX: 'auto' }}>
        <table style={{ borderCollapse: 'collapse', width: '100%', minWidth: 640 }}>
          <thead>
            <tr>
              {cols.map((c) => (
                <th key={c.k} style={{ padding: '8px 10px', fontSize: 12, fontWeight: 700, color: '#FFFFFF', background: couleur, textAlign: 'left' }}>{c.t}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {lignes.map((r) => (
              <tr key={r} style={{ borderTop: '1px solid #F1F3F5' }}>
                {cols.map((c) => (
                  <td key={c.k} style={{ padding: '5px 7px', verticalAlign: 'top' }}>
                    <input type="text" disabled={verrouille} value={saisies[`${a.id}.r${r}.${c.k}`] ?? ''} onChange={(e) => set(`${a.id}.r${r}.${c.k}`, e.target.value)} style={inp} />
                  </td>
                ))}
              </tr>
            ))}
