            ...
            {mission ? (
              <>
                ...
                {(() => {
                  ...
                  const fond = eclaircir(scenario.couleur, 0.85)
                  const bord = eclaircir(scenario.couleur, 0.5)
                  const titreBloc = (txt: string) => (
                    <div style={{ fontSize: 13, fontWeight: 700, color: '#16456E', margin: '18px 0 8px', display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ width: 7, height: 16, background: scenario.couleur, borderRadius: 2 }} />{txt}
                    </div>
                  )
                  return (
                    <div>
                      ...
                      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 6 }}>
                        <span style={{ background: fond, border: `1px solid ${bord}`, borderRadius: 8, padding: '6px 12px', fontSize: 12 }}><b>Durée</b> : {d.dureeTotale}</span>
                        <span style={{ background: fond, border: `1px solid ${bord}`, borderRadius: 8, padding: '6px 12px', fontSize: 12 }}><b>Activités</b> : {d.nbActivites}</span>
                        <span style={{ background: fond, border: `1px solid ${bord}`, borderRadius: 8, padding: '6px 12px', fontSize: 12 }}><b>Questions</b> : {d.nbQuestions}</span>
                      </div>

                      {d.contexte && (
                        <>
                          {titreBloc('Contexte')}
                          <p style={{ fontSize: 13, color: '#374151', lineHeight: 1.6, margin: 0 }}>{d.contexte}</p>
                        </>
                      )}

                      {d.competence && (
                        <>
                          {titreBloc('Compétence travaillée')}
                          <div style={{ fontSize: 13, color: '#374151', lineHeight: 1.6 }}>
                            <div style={{ fontWeight: 700 }}>{d.competence.groupe}</div>
                            <div style={{ fontWeight: 600 }}>{d.competence.intitule}</div>
                            <div>{d.competence.detail}</div>
                          </div>
                        </>
                      )}
                      ...
                      {titreBloc('Déroulé pédagogique')}
                      <div style={{ overflowX: 'auto', border: `1px solid ${bord}`, borderRadius: 8 }}>
                        <table style={{ borderCollapse: 'collapse', width: '100%', minWidth: 520 }}>
                          <thead>
                            <tr>
                              {['Phase', 'Durée', 'Modalité', 'Supports'].map((h) => (
                                <th key={h} style={{ background: scenario.couleur, color: couleurTexteSur(scenario.couleur), fontSize: 12, fontWeight: 700, textAlign: 'left', padding: '8px 10px', whiteSpace: 'nowrap' }}>{h}</th>
                              ))}
                            </tr>
                          </thead>
                          <tbody>
                            {d.phases.map((p, i) => (
                              <tr key={i} style={{ background: i % 2 === 0 ? '#FFFFFF' : fond }}>
                                <td style={{ fontSize: 12, color: '#1F2933', padding: '8px 10px', borderTop: `1px solid ${bord}`, fontWeight: 600 }}>{p.phase}</td>
                                <td style={{ fontSize: 12, color: '#374151', padding: '8px 10px', borderTop: `1px solid ${bord}`, whiteSpace: 'nowrap' }}>{p.duree}</td>
                                <td style={{ fontSize: 12, color: '#374151', padding: '8px 10px', borderTop: `1px solid ${bord}`, whiteSpace: 'nowrap' }}>{p.modalite}</td>
                                <td style={{ fontSize: 12, color: '#374151', padding: '8px 10px', borderTop: `1px solid ${bord}` }}>{p.supports}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
